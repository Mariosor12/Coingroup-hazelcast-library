...
function GetLastDataCandle(json, id_market, candle) {
  ...
  var data_length;
  var data;
  path = _jsonpath["default"].query(json, "$..id_market");

  if (id_market == path) {
    data_length = _jsonpath["default"].query(json, "$.." + candle + ".max_length");
    data_array = _jsonpath["default"].query(json, "$.." + candle + ".data_array[-1:]");
    data = {
...
function GetOneDataCandle(json, id_market, candle, index) {
  ...
  if (id_market == path) {
    data_length = _jsonpath["default"].query(json, "$.." + candle + ".max_length");
    data_array = _jsonpath["default"].query(json, "$.." + candle + ".data_array[" + index + "]");
    data = {
      data_length: data_length[0],
      data_array: data_array
    };
...
function GetDataAttrCandle(json, id_market, candle, index, name) {
  ...
  if (id_market == path) {
    data_length = _jsonpath["default"].query(json, "$.." + candle + ".max_length");
    data_array = _jsonpath["default"].query(json, "$.." + candle + ".data_array[" + index + "]." + name);
    ohlc = _jsonpath["default"].query(json, "$.." + candle + ".data_array[" + index + "].ohlc." + name);
...
function GetLastDataAttrCandle(json, id_market, candle, name) {
  ...
  if (id_market == path) {
    data_length = _jsonpath["default"].query(json, "$.." + candle + ".max_length");
    data_array = _jsonpath["default"].query(json, "$.." + candle + ".data_array[-1:]." + name);
    ohlc = _jsonpath["default"].query(json, "$.." + candle + ".data_array[-1:].ohlc." + name);
...
function GetThreeLastDataCandle(json, id_market, candle) {
  ...
  if (id_market == path) {
    data_length = _jsonpath["default"].query(json, "$.." + candle + ".max_length");
    data_array = _jsonpath["default"].query(json, "$.." + candle + ".data_array[-3:]");
    data = {
      data_length: data_length[0],
      data_array: data_array
    };
